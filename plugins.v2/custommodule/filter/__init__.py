...
class FilterGenModule(_ModuleBase):
    # 规则解析器
    ...
    def __match_rule(self, torrent: TorrentInfo, rule_name: str) -> bool:
        ...
        uploadvolumefactor = self.rule_set[rule_name].get("uploadvolumefactor")
        # 发布时间规则
        ...
        if uploadvolumefactor is not None:
            if torrent.uploadvolumefactor != uploadvolumefactor:
                # FREE规则不匹配
                logger.debug(
                    f"种子 {torrent.site_name} - {torrent.title} 上传倍数值 {torrent.uploadvolumefactor} 不是 {uploadvolumefactor}")
                return False
        if pubdate:
            # 种子发布时间
